...
```{r clean our data}
tidy_words <- data %>%
  gather('word_type', 'word_str', starts_with('word_')) %>%
  select(-starts_with('query'), -starts_with('polit'), -starts_with('stigma'))
tidy_words$word_type = gsub("word_", "", tidy_words$word_type)
tidy_words$word_str = sapply(tidy_words$word_str, tolower)
```
