...
```{r clean our data}
tidy_words <- data %>%
  gather('word_type', 'word_str', starts_with('word_'), -contains('filter')) %>%
  select(-starts_with('query'), -starts_with('polit'), -starts_with('stigma'), -contains('filter'))
tidy_words$word_type = gsub("word_", "", tidy_words$word_type)
tidy_words$word_str = tolower(tidy_words$word_str)
```
...
```{r analyze mean}
mean_results = tidy_words %>%
  group_by(condition) %>%
  summarise("proportion_match" = mean(word_str==word_type))
```
Ok, that's great, but what if we want to see the condition different for each subject?

```{r find mean proportion m}
mean_results_by_subj = tidy_words %>%
  group_by(condition, V1) %>%
  summarise("proportion_match" = mean(word_str==word_type))
```
...
tidy_words %>%
  group_by(condition, V1) %>%
  summarise("proportion_match" = mean(word_str==word_type)) %>%
  summarise('sd_subjmean' = sd(proportion_match))

mean_results_by_subj %>%
  group_by(condition) %>%
  summarise(mean_sd = sd(proportion_match))
```
Ok, let's try and plot that...

```{r meanresults boxplot}
mean_results_by_subj %>%
  ggplot(aes(x = condition, y = proportion_match)) + 
  geom_boxplot()
```
...
```{r tally gender}
gender_tally = tally(group_by(data,gender))
```
...
```{r remove class}
interaction_data = mutate(tidy_words, 'match' = word_type==word_str)
interaction_data = interaction_data %>% 
  filter(gender != "Other")

interaction_data %>%
  group_by(condition, V1, gender) %>%
  summarise("proportion_match" = mean(word_str==word_type)) %>%
  ggplot(aes(x = condition, y = proportion_match, color=gender)) + 
      geom_boxplot()
```
